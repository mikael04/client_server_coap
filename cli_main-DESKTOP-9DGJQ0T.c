	...
	void add_option (coap_option_t opt, char *op_num, char *op_conteudo, int numopt, int *option_running_delta)
	{
		char *end;
		char aux[30] = "";
	...
	#endif
		opt.num = (uint8_t *) end;
		opt.buf.len = strlen(aux);
		opt.buf.p = (uint8_t *) aux;
		printf("op_conteudo = %s\n", aux);
		...
		while (j<argc)
		{
			...
			else if(strcmp(argv[j], "-op") == 0)
			{
				...
				else
				{
					...
					printf("4op) j = %d\n", j);
	#endif			
					add_option(pkt->opts[cont_aux], argv[j+1], argv[j+2], cont1->cont_op, &option_delta);
